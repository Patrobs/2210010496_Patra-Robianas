...
class Ui_Form(object):
    ...
    def insertmahasiswa(self):
        try:
            ...
            if not mydb:
                return
            cursor = mydb.cursor()
            sql = "INSERT INTO mahasiswa (npm, nama, kelas, jurusan,  ttl) VALUES (%s, %s, %s, %s, %s)"
            val = (
    ...
    def ubahmahasiswa(self):
        try:
            ...
            if not mydb:
                return
            cursor = mydb.cursor()
            sql = "UPDATE mahasiswa SET nama = %s, kelas = %s, jurusan = %s, ttl = %s WHERE npm = %s"
            val = (
                self.lineEditNAMA.text(),
                self.lineEditKelas.text(),
                self.lineEditJurusan.text(),
    ...
    def on_table_item_clicked(self, row, column):
        try:
            ...
            ttl = self.tableWidget.item(row, 4).text()
            ...
            self.lineEditTanggalLahir.setText(ttl)
